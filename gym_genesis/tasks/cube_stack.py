...
class CubeStackV2:
    ...
    def _build_scene(self, num_envs, env_space):
    ...
    def reset(self):
        ...
        x1 = self._random.uniform(0.45, 0.75)
        y1 = self._random.uniform(-0.2, 0.2)
        z = 0.02
        ...
        x2 = self._random.uniform(0.3, 0.7)
        ...
        if hasattr(self, "distractor_cubes"):
            for cube in self.distractor_cubes:
                xd = self._random.uniform(0.3, 0.7)
                yd = self._random.uniform(-0.3, 0.3)
    ...
    def compute_reward(self):
        # Get cube_1 and cube_2 positions (single env)
        pos_1 = self.cube_1.get_pos()  # shape (3,)
        pos_2 = self.cube_2.get_pos()  # shape (3,)

        # XY distance between cube centers
        xy_dist = torch.norm(pos_1[:2] - pos_2[:2])  # no dim needed

        # Z height difference
        z_diff = pos_1[2] - pos_2[2]

        # Reward: if cube_1 is well-aligned above cube_2
        reward = float((xy_dist < 0.05) and (z_diff > 0.03))  # scalar reward (0.0 or 1.0)
        return reward


    def get_obs(self):
        # === Agent (robot) state ===
        eef_pos = self.eef.get_pos().cpu().numpy().squeeze()     # (3,)
        eef_rot = self.eef.get_quat().cpu().numpy().squeeze()    # (4,)
        gripper = self.franka.get_dofs_position().cpu().numpy().squeeze()[7:9]  # (2,)

        # === Object state ===
        cube1_pos = self.cube_1.get_pos().cpu().numpy().squeeze()  # (3,)
        cube1_rot = self.cube_1.get_quat().cpu().numpy().squeeze()  # (4,)
        cube2_pos = self.cube_2.get_pos().cpu().numpy().squeeze()  # (3,)

        # === Privileged Features ===
        diff = eef_pos - cube1_pos                                # (3,)
        dist = np.linalg.norm(diff, keepdims=True)                # (1,)
