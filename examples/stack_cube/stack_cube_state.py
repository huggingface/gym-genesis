...
def expert_policy(robot, obs, stage):
    ...
    current_pos = robot.get_link("hand").get_pos(envs_idx=torch.arange(B, device=gs.device))  # (B, 3)
    cart_wps = []
...
for ep in range(50):
    ...
    for stage in ["hover", "grasp", "lift", "place", "release"]:
        ...
        for action in action_path:  # each action is (B, 9)
            ...
            all_agent_states.append(obs["agent_pos"].detach().cpu().numpy())
            all_env_states.append(obs["environment_state"].detach().cpu().numpy())
